...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow("BREYE FOKA FIRST SDL2 CODE", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                                          900, 650, SDL_WINDOW_SHOWN | SDL_WINDOW_OPENGL);
    SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);

    // Initial renderer color
    SDL_SetRenderDrawColor(renderer, randCol(), randCol(), randCol(), 123);
